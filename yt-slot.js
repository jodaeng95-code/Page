  ...
  class YtSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (stored && stored.id) {
        ...
        // Fallback: use the url="" attribute baked into the HTML, so the
        // thumbnail always shows for every visitor, with no localStorage needed.
        const attrUrl = this.getAttribute('url');
        const ytId = parseYtId(attrUrl);
        if (ytId) {
